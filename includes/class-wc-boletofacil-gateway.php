        ...
        if ( 'true' == $responseData->{'success'} ) {
            ...
            if ( isset( $data->{'code'} ) ) {
                ...
                if ('transparent' !== $this->method) {
                    ...
                    if ('CREDIT_CARD' == $_POST['pmethod']) $is_creditcard = true;
                ...
                add_post_meta($order->get_id(), 'boletofacil_is_transparent', $is_transparent);
                return true;
            }
        } else {
...
//    			$args['creditCardSecurityCode']    = preg_replace('/[^0-9]/', '', $_POST['card_security_code']);
    ...
    public function thankyou_page( $order_id ) {
        ...
        $is_transparent = get_post_meta($order_id, 'boletofacil_is_transparent', true);
        ...
        if (!$is_credit_card) {
            ...
            if($is_transparent) {
                ...
                $message .= 'Para concluir o pagamento por favor clique no link "Visualizar Cobrança". <br />';
            }
        ...
        if (!empty($url) && $is_credit_card && !$is_transparent ) {
            $html .= sprintf( '<a class="button" href="%s" target="_blank">%s</a>', $url, 'Visualizar Cobrança' );
        }
        $html .= '</div>';
        echo $html;
    }
    ...
    public function email_instructions( $order, $sent_to_admin ) {
        ...
        $is_transparent = get_post_meta($order->get_id(), 'boletofacil_is_transparent', true);
        if (!$is_credit_card) {
            $message .= '<strong> Atenção! </strong> Você não receberá o boleto pelos Correios. <br />';
            $message .= 'Por favor clique no link "Imprimir boleto" a seguir e pague o boleto em seu Internet Banking. <br />';
            $message .= 'Se preferir, imprima e pague em uma agência bancária ou casa lotérica. <br />';
